...
import models.subscription.Product._
...
import models.subscription.{Benefit, BillingPeriod}
    ...
    new SubPlanReads[PaidSubscriptionPlan[P, C]] {
      override def read(p: ProductIds, z: SubscriptionZuoraPlan, c: CatalogZuoraPlan): ValidationNel[String, PaidSubscriptionPlan[P, C]] =
        (chargeListReads.read(c.benefits, z.charges.list.toList) |@| productReads.read(p, z, c)) { case (charges, product) =>
          val highLevelFeatures = z.features.map(models.subscription.Subscription.Feature.fromRest)
          ...
          case (charges: ChargeList, product: P) =>
            (charges match {
              ...
              case paidChargeList: PaidChargeList =>
                val highLevelFeatures = subZuoraPlan.features.map(models.subscription.Subscription.Feature.fromRest)
                PaidSubscriptionPlan(
                  subZuoraPlan.id,
                  catZuoraPlan.id,
                  catZuoraPlan.name,
