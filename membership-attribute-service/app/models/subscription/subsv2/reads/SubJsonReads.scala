...
import models.subscription.{BillingPeriod, PricingSummary}
  ...
  def subscriptionReads[P <: SubscriptionPlan.AnyPlan](
    ...
    override def reads(json: JsValue): JsResult[NonEmptyList[P] => Subscription[P]] = {
      ...
      json match {
        case o: JsObject =>
          (
            (__ \ "id").read[String].map(subscription.Subscription.Id) and
              (__ \ "subscriptionNumber").read[String].map(subscription.Subscription.Name) and
              (__ \ "accountId").read[String].map(subscription.Subscription.AccountId) and
          ...
          )(subscription.subsv2.Subscription.partial[P](hasPendingFreePlan) _).reads(o)
