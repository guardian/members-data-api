...
import models.subscription.promo._
...
class PromoService(promos: PromotionCollection, discounter: Discounter)(implicit ec: ExecutionContext) {
  ...
  /** Validate a promotion
    */
  def validate[C >: Both <: PromoContext](promoCode: PromoCode, country: Country, prpId: ProductRatePlanId, now: DateTime = DateTime.now)(implicit
      validator: PromotionValidator[C],
  ): PromoError \/ ValidPromotion[C] = {
    for {
  ...
  /** Validate lots of promo codes If all the promo codes are None, a Right of None will be returned otherwise the first successful promo code or
    * error will come back
    */
  def validateMany[C >: Both <: PromoContext](country: Country, prpId: ProductRatePlanId, now: DateTime = DateTime.now)(
      codes: Option[PromoCode]*,
  )(implicit matcher: PromotionValidator[C]): PromoError \/ Option[ValidPromotion[C]] = {

    codes
      .flatMap(_.toSeq)
      .map(validate[C](_, country, prpId))
