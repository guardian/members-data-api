...
class PaymentUpdateController(commonActions: CommonActions, override val controllerComponents: ControllerComponents) extends BaseController {
  ...
  def updateCard(subscriptionName: String) = AuthAndBackendViaIdapiAction(Return401IfNotSignedInRecently).async { implicit request =>
    ...
    (for {
      ...
      stripeDetails <- EitherT(
        Future.successful(updateForm.orElse(legacyForm) \/> "no 'stripePaymentMethodID' and 'stripePublicKey' submitted with request")
      )
      ...
      subscription <- EitherT(
        tp.subService
          ...
          .map(subscriptionSelector(Some(memsub.Subscription.Name(subscriptionName)), s"the sfUser $sfUser"))
      )
      stripeService <- EitherT(
        Future.successful(tp.stripeServicesByPublicKey.get(stripePublicKey)).map(_ \/> s"No Stripe service for public key: $stripePublicKey")
      )
      updateResult <- EitherT(
        setPaymentCardFunction(subscription.accountId, stripeCardIdentifier, stripeService).map(
          _ \/> "something was missing when attempting to update payment card in Zuora"
        )
      )
  ...
  def updateDirectDebit(subscriptionName: String) = AuthAndBackendViaIdapiAction(Return401IfNotSignedInRecently).async { implicit request =>
    ...
    (for {
      ...
      freshDefaultPaymentMethodOption <- EitherT(
