...
object RequestRunners {
  ...
  /** This is a runner that times how long calls takes with cloudwatch and logs details of them. It also allows the read timeout for the OkHttpClient
    * to be configured. It seems Kleisli is basically a name for a ReaderT, in that what we want to do is return a reader monad that given a string
    * for the request ID will run the request and log its details against that request ID
    */
  def configurableLoggingRunner(timeout: Duration, metrics: StatusMetrics)(implicit
      ec: ExecutionContext,
  ): Request => ReaderT[String, Future, OkResponse] = r =>
    Kleisli { requestId =>
      val resp = Timing.record(metrics, requestId) {
        val seconds: Int = timeout.toSeconds.toInt
