...
object SuspensionService {
  ...
  case class HolidayRenewCommand(sub: models.subscription.subsv2.Subscription[SubscriptionPlan.Delivery])
...
class SuspensionService[M[_]: Monad](plans: HolidayRatePlanIds, simpleRest: SimpleClient[M]) {
  ...
  def subThatNeedsRenewing(sub: models.subscription.subsv2.Subscription[SubscriptionPlan.Delivery], holiday: PaymentHoliday) = {
    if (holiday.finish.isAfter(sub.termEndDate)) Some(sub) else None
  }

  def renewIfNeeded(sub: models.subscription.subsv2.Subscription[SubscriptionPlan.Delivery], holiday: PaymentHoliday): M[\/[String, Unit]] = {
