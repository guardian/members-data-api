...
object ChooseStripe {
  def createFor(ukStripeConfig: StripeServiceConfig, auServiceConfig: StripeServiceConfig)(implicit ec: ExecutionContext) = {
    ...
    val ukStripeService: StripeService = new StripeService(ukStripeConfig, RequestRunners.futureRunner)
    val auStripeService: StripeService = new StripeService(auServiceConfig, RequestRunners.futureRunner)
