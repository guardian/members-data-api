...
import com.gu.memsub.subsv2.reads.SubJsonReads.{multiSubJsonReads, subZuoraPlanListReads, subscriptionReads}
...
import com.gu.memsub.subsv2.{CatalogZuoraPlan, PaidSubscriptionPlan, Subscription, SubscriptionPlan, SubscriptionZuoraPlan}
import com.gu.monitoring.SafeLogger
import com.gu.salesforce.ContactId
import com.gu.zuora.rest.SimpleClient
import org.joda.time.{LocalDate, LocalTime}
import play.api.libs.json.{JsResult, JsSuccess, JsValue, Reads => JsReads}
import scalaz._
import scalaz.syntax.all._
import scalaz.syntax.std.either._
import scalaz.syntax.std.option._
import services.ZuoraSubscriptionService.{CatalogMap, SoapClient}
...
object ZuoraSubscriptionService {
...
class ZuoraSubscriptionService(pids: ProductIds, futureCatalog: => Future[CatalogMap], rest: SimpleClient[Future], soap: SoapClient)(implicit
    t: Monad[Future],
    ec: ExecutionContext,
  ...
  )(implicit reads: SubPlanReads[AnyPlan]): Future[Option[Subscription[AnyPlan]]] = {
    ...
    futureSubJson.flatMap { subJson =>
      futureCatalog.map { catalog =>
        ...
        val allSubscriptionsForSubscriberName = subJson.flatMap { jsValue =>
          SubscriptionTransform.getSubscription[AnyPlan](catalog, pids)(jsValue).withTrace("getAllValidSubscriptionsFromJson")
        }
  ...
  def since(onOrAfter: LocalDate)(contact: ContactId)(implicit reads: SubPlanReads[AnyPlan]): Future[List[Subscription[AnyPlan]]] =
    subscriptionsForContact(SubscriptionTransform.getSubscriptionsActiveOnOrAfter[AnyPlan](onOrAfter))(contact)
  ...
  def decideCancellationEffectiveDate(
      ...
      today: LocalDate = LocalDate.now(),
  )(implicit reads: SubPlanReads[AnyPlan]): EitherT[String, Future, Option[LocalDate]] = {
    EitherT(
