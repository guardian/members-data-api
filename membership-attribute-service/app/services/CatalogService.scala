...
object FetchCatalog {
  ...
  def fromZuoraApi(httpClient: SimpleClient[Future]): Future[String \/ JsValue] =
  ...
  def fromS3(zuoraEnvironment: String, s3Client: AmazonS3 = AwsS3.client): Future[String \/ JsValue] = {
    val catalogRequest = new GetObjectRequest(s"gu-zuora-catalog/PROD/Zuora-${zuoraEnvironment}", "catalog.json")
    AwsS3.fetchJson(s3Client, catalogRequest).point[Future](implicitly[Monad[Future]])
  }
}

class CatalogService(productIds: ProductIds, fetchCatalog: Future[String \/ JsValue], unsafeGet: Future[Catalog] => Catalog, stage: String) {
  import scala.concurrent.ExecutionContext.Implicits.global
  import scalaz.std.scalaFuture._
  ...
  def many[P <: CatalogPlan[Product, ChargeList, Status]](plans: List[CatalogZuoraPlan], name: String)(implicit
    ...
    val failures = parsed
      .filter { case (_, validation) => validation.isFailure }
      .map { case (id, err) => (id, err.swap.map(_.list.toList).getOrElse(List.empty).mkString(", ")) }
      .toMap

    parsed
      .map { case (_, validation) => validation }
  ...
  def joinUp: Future[String \/ List[CatalogZuoraPlan]] = (for {
    catalog <- EitherT[String, Future, JsValue](fetchCatalog)
    catalogPlans <- EitherT[String, Future, List[CatalogZuoraPlan]](
      Json.fromJson[List[CatalogZuoraPlan]](catalog).asEither.disjunction.leftMap(_.toString).point[Future],
    )
  ...
  lazy val catalog: Future[NonEmptyList[String] \/ Catalog] = (for {
    ...
    catalog <- EitherT(validatePlans(plans).toDisjunction.point[Future])
  } yield catalog).run
