...
import monitoring.{CreateMetrics, SafeLogger, SalesforceMetrics, Timing}
import okhttp3._
...
class Scalaforce(
    ...
    val createMetrics: CreateMetrics,
)(implicit ec: ExecutionContext)
    extends LazyLogging {

  val metrics = createMetrics.forService("Salesforce")
  ...
  private def logResponseCode(req: Request, response: Response): Unit = {
    val statusClass = response.code() / 100
    metrics.incrementCount(s"${statusClass}XX-response-code ${req.method()}")
  }
  ...
  object Query {
    def execute(query: String): Future[\/[String, JsValue]] = {
      ...
      metrics
        .measureDuration("Execute query")(get(path))
        .map { response =>
  ...
  object Contact {
    def read(key: String, id: String): Future[\/[String, Option[JsValue]]] = {
      ...
      metrics
        .measureDuration("Read Contact")(get(path))
        .map { response =>
          val bodyString = response.body().string() // out here to make sure connection is closed in all cases
          response.code() match {
    ...
    def upsert(upsertKey: Option[(String, String)], data: JsObject): Future[SFContactRecord] = {
      ...
      metrics
        .measureDuration("Upsert Contact") {
    ...
    private def update(id: SFContactId, json: JsValue): Future[Unit] =
      metrics
        .measureDuration("Update Contact") {
  ...
  private def fetchAndStoreAuth() = authorize.onComplete {
    ...
    case Failure(ex) =>
      ...
      metrics.incrementCount("auth-error")
  }
