...
import subscriptions.CAS.CASSuccess
...
class CASDeserializerTest extends Specification {
  "CASDeserializer" should {
    "deserialize a success" in {
      ...
      successOpt mustEqual Some(CASSuccess("sub", Some("provider"), "2030-02-24", Some("XXX"), "CONTENT"))
    }
    ...
    "deserialize a success with no optional field" in {
      ...
      successOpt mustEqual Some(CASSuccess("sub", None, "2030-02-24", None, "CONTENT"))
    }

    "deserialize an error" in {
      val errorOpt = Resource.getJson("cas/error.json").asOpt[CASError]
      errorOpt mustEqual Some(CASError("Unknown subscriber", Some(-90)))
    }
  }
}
