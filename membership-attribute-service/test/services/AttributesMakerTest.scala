...
class AttributesMakerTest(implicit ee: ExecutionEnv) extends Specification with SubscriptionTestData {
  ...
  "zuoraAttributes" should {
    ...
    "return attributes when digipack sub" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(digipack))),
    ...
    "return attributes when one of the subs has a digital benefit" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(sunday)), AccountWithSubscriptions(anotherAccountSummary, List(sundayPlus))),
        paymentMethodResponseNoFailures,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "return none when only sub is expired" in {
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(expiredMembership))),
        paymentMethodResponseNoFailures,
        referenceDate
      )
    ...
    "return attributes when there is one membership sub" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(membership))),
        paymentMethodResponseNoFailures,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "return attributes when one sub is expired and one is not" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(
          AccountWithSubscriptions(accountObjectWithZeroBalance, List(expiredMembership)),
    ...
    "return attributes when one sub is a recurring contribution" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(contributor))),
        paymentMethodResponseNoFailures,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "return attributes relevant to both when one sub is a contribution and the other a membership" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        ...
        List(
          AccountWithSubscriptions(accountObjectWithZeroBalance, List(contributor)),
          AccountWithSubscriptions(accountObjectWithZeroBalance, List(membership))
        ),
        paymentMethodResponseNoFailures,
        referenceDate
      )
    ...
    "return attributes when the membership is a friend tier" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithZeroBalance, List(friend))),
        paymentMethodResponseNoFailures,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "return alertAvailableFor=membership for an active membership in payment failure" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithBalance, List(membership))),
        paymentMethodResponseRecentFailure,
        referenceDate
      )
      result must be_==(expected).await
    }
    "return alertAvailableFor=contribution for an active recurring contribution in payment failure" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithBalance, List(contributor))),
        paymentMethodResponseRecentFailure,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "return alertAvailableFor=contribution when there is an active contribution and membership, both in payment failure" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(
          AccountWithSubscriptions(accountObjectWithBalance, List(membership)),
          AccountWithSubscriptions(accountObjectWithBalance, List(contributor))
        ),
        paymentMethodResponseRecentFailure,
        referenceDate
      )
    ...
    "still return alertAvailableFor=contribution when there is an active contribution and paper sub, both in payment failure" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(AccountWithSubscriptions(accountObjectWithBalance, List(sunday)), AccountWithSubscriptions(accountObjectWithBalance, List(contributor))),
        paymentMethodResponseRecentFailure,
        referenceDate
      )
      result must be_==(expected).await
    ...
    "still return alertAvailableFor=contribution when there is an active contribution and guardianWeekly sub, both in payment failure" in {
      ...
      val result = AttributesMaker.zuoraAttributes(
        identityId,
        List(
          AccountWithSubscriptions(accountObjectWithBalance, List(guardianWeekly)),
          AccountWithSubscriptions(accountObjectWithBalance, List(contributor))
        ),
        paymentMethodResponseRecentFailure,
        referenceDate
