...
import models.subscription.Benefit.{Contributor, Digipack, Friend, Partner, Supporter, Weekly}
...
import models.subscription.subsv2.SubscriptionPlan.{AnyPlan, Partner}
import models.subscription.subsv2.reads.ChargeListReads.{anyBpReads, readAnyProduct}
...
import services.subscription.{SubscriptionService, ZuoraSubscriptionService}
...
class SubscriptionServiceTest extends Specification {
  ...
  val soapClient: SubscriptionService.SoapClient = _ =>
    Future.successful(
      List(
        AccountId("foo"),
        AccountId("bar"),
  ...
  val subscriptions: Request => Future[Response] = request =>
    Future.successful(request.url().uri().getPath match {
  ...
  val rc = SimpleClient(ZuoraRestConfig("TESTS", "https://localhost", "foo", "bar"), subscriptions, CreateNoopMetrics)
  val service = new ZuoraSubscriptionService(Fixtures.productIds, cat, rc, soapClient)
  ...
  "Current Plan" should {
    ...
    "tell you a downgraded plan to friend, friend is current if we're after the plan end date" in {
      ...
      val result = GetCurrentPlans(subs, referenceDate + richInt(1).day).map(_.head.id.get)
    ...
    "tell you you are no longer a supporter if your subscription is after an end date" in {
      ...
      val subs = toSubscription(isCancelled = false)(plans)
      val result = GetCurrentPlans(subs, referenceDate + richInt(1).day).leftMap(_.contains("ended"))
      result mustEqual -\/(true) // not helpful
    }

    "if you've cancelled and then signed up with a different tier, should return the new tier on day 1" in {
      val plans =
        NonEmptyList(partnerPlan(referenceDate, referenceDate - 1.year), supporterPlan(referenceDate + richInt(1).day, referenceDate + 1.year))
      val subs = toSubscription(isCancelled = false)(plans)
      val result = GetCurrentPlans(subs, referenceDate + richInt(1).day).map(_.head.id.get)
      result mustEqual \/-("idSupporter")
    }
    ...
    "if you're in a free trial of digipack, tell you you're already a digipack subscriber" in {
      ...
      val firstPayment = referenceDate + richInt(14).days
  ...
  "Subscription service" should {
    ...
    "Be able to fetch a friend subscription" in {
      val sub = waitFor(service.get[SubscriptionPlan.Friend](Name("1234")))
      ...
      sub.map(_.name) mustEqual Some(Name("subscriptionNumber"))
    }

    "Give you back a none in the event of the sub not existing" in {
      waitFor(service.get[SubscriptionPlan.Friend](Name("foo"))) mustEqual None
    }
    ...
    "Leverage the soap client to fetch subs by contact ID" in {
      val subs = waitFor(service.current[SubscriptionPlan.Friend](contact))
      subs.headOption.map(_.name) mustEqual Some(Name("subscriptionNumber")) // what is in the config file
    }

    "Be able to fetch subs where term ends after the specified date" in {
      val currentSubs = waitFor(service.current[SubscriptionPlan.Friend](contact))
      val sinceSubs = waitFor(service.since[SubscriptionPlan.Friend](1 Jan 2020)(contact))
    ...
    "Allow you to fetch one of two subscriptions across product families" in {
      val subs = waitFor(service.either[SubscriptionPlan.Member, SubscriptionPlan.ContentSubscription](contact))
      subs.toOption.flatMap(_.flatMap(_.swap.toOption).map(_.name)) mustEqual Some(
        Name("subscriptionNumber"),
      ) // what is in the config file
    }

    "Allow you to fetch one of two subscriptions" in {
      val subs = waitFor(service.either[SubscriptionPlan.Friend, SubscriptionPlan.Digipack](contact))
      subs.toOption.flatMap(_.flatMap(_.swap.toOption).map(_.name)) mustEqual Some(
        Name("subscriptionNumber"),
      ) // what is in the config file
    }
    ...
    "Allow you to fetch an upgraded subscription" in {
      waitFor(service.get[Partner](Name("A-S00063478"))).map(GetCurrentPlans(_, referenceDate).map(_.head.charges.benefit)) mustEqual Some(
        \/-(Partner),
      )
    }

    "Decided cancellation effective date should be None if within lead time period before first fulfilment date" in {
      waitFor(service.decideCancellationEffectiveDate[AnyPlan](Name("A-lead-time")).run) mustEqual \/.right(None)
    }
    ...
    "Deciding cancellation effective date should error because Invoiced period has started today, however Bill Run has not yet completed" in {
      waitFor(
        service
          ...
          .run,
      ) mustEqual \/.left("Invoiced period has started today, however Bill Run has not yet completed (it usually runs around 6am)")
    }

    "Decided cancellation effective date should be end of 6-for-6 invoiced period if user has started 6-for-6" in {
      waitFor(
        service.decideCancellationEffectiveDate[AnyPlan](Name("A-segment-6for6"), today = LocalDate.parse("2020-10-02")).run,
      ) mustEqual \/.right(
