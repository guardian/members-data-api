...
import _root_.services.zuora.soap.models.Queries.PreviewInvoiceItem
import models.subscription.Benefit.Digipack
import models.subscription.{Benefit, BillingSchedule}
import models.subscription.BillingSchedule.{Bill, BillItem, rolledUp}
import models.subscription.Subscription.ProductRatePlanChargeId
import scalaz.Scalaz.ToNelOps
...
class BillingScheduleTest extends Specification {
  ...
  "getBillingSchedule" should {
    ...
    "Merge credits occurring half way through a month into the subsequent bill" in {
      ...
      BillingSchedule.fromPreviewInvoiceItems(_ => None)(withHolidays) mustEqual Some(
        BillingSchedule(
          NonEmptyList(
    ...
    "Include any products found with the productFinder in the bill items" in {
      ...
      BillingSchedule.fromPreviewInvoiceItems(discerningFinder)(items) mustEqual Some(
        BillingSchedule(
          NonEmptyList(
