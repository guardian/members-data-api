...
class SubscriptionServiceTest extends Specification {
  ...
  val soapClient: SubscriptionService.SoapClient[Id] = _ =>
    List(
      memsub.Subscription.AccountId("foo"),
      memsub.Subscription.AccountId("bar"),
    )

  val subscriptions: Request => Id[Response] = a =>
    a.url().uri().getPath match {
  ...
  "Subscription service" should {
    ...
    "Allow you to fetch one of two subscriptions across product families" in {
      ...
      subs.toOption.flatMap(_.flatMap(_.swap.toOption).map(_.name)) mustEqual Some(
        memsub.Subscription.Name("subscriptionNumber"),
