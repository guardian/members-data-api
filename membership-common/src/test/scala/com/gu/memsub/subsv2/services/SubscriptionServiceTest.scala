...
class SubscriptionServiceTest extends Specification {
  ...
  "Subscription service" should {
    ...
    "Be able to fetch a friend subscription" in {
      ...
      sub.map(_.plan) mustEqual Some(FreeSubscriptionPlan(RatePlanId("id"), prpId, "foo", "", "Friend", "Membership", Product.Membership, FreeCharge(Friend, Set(GBP)), start = 12 Aug 2016, end = 13 Aug 2016))
    ...
    "Allow you to fetch an upgraded subscription" in {
      service.get[SubscriptionPlan.Partner](Name("A-S00063478")).map(GetCurrentPlans(_, referenceDate).map(_.head.charges.benefit)) mustEqual Some(\/-(Partner))
    }

    "Decided cancellation effective date should be None if within lead time period before first fulfilment date" in {
      service.decideCancellationEffectiveDate[AnyPlan](Name("A-lead-time"))
        .run mustEqual \/.right(None)
    }
    ...
    "Deciding cancellation effective date should error because Invoiced period has started today, however Bill Run has not yet completed" in {
      service.decideCancellationEffectiveDate[AnyPlan](Name("GW-before-bill-run"), LocalTime.parse("01:00"), LocalDate.parse("2020-10-02"))
    ...
    "Decided cancellation effective date should be end of 6-for-6 invoiced period if user has started 6-for-6" in {
      service.decideCancellationEffectiveDate[AnyPlan](Name("A-segment-6for6"), today = LocalDate.parse("2020-10-02"))
        .run mustEqual \/.right(Some(LocalDate.parse("2020-11-13")))
    }
    ...
    "Deciding cancellation effective date should error because Invoiced period exists, and Bill Run has completed, but today is after end of invoice date" in {
      service.decideCancellationEffectiveDate[AnyPlan](Name("GW-stale-chargeThroughDate"), service.BillRunCompletedByTime.plusHours(1), LocalDate.parse("2020-06-20").plusDays(1))
        .run mustEqual \/.left("chargedThroughDate exists but seems out-of-date because bill run should have moved chargedThroughDate to next invoice period. Investigate ASAP!")
    }
  }
}
