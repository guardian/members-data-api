...
abstract class WebServiceHelper[T, Error <: Throwable](implicit ec: ExecutionContext) extends SafeLogging {
  ...
  /** Manipulate the request before it is executed. Generally used to add any authentication settings the web services requires (e.g. add an
    ...
    */
  def wsPreExecute(req: Request.Builder)(implicit logPrefix: LogPrefix): Request.Builder = req
  ...
  /** Send a request to the web service and attempt to convert the response to an A
    ...
    *   The request to send
    * @param reads
    *   A Reader to convert JSON to A
    * @param error
    *   A Reader to convert JSON to Error
    * @tparam A
