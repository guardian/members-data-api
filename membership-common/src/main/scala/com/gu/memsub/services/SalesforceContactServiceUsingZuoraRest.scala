...
object SalesforceContactServiceUsingZuoraRest {
  ...
  def getPersonContactForAccountId(
      salesforceAccountId: String,
  )(implicit simpleContactRepository: SimpleContactRepository, ctx: ExecutionContext): Future[Contact] = {
  ...
  def zuoraAccountFromSub(subscription: Subscription[AnyPlan])(implicit
  ...
  def getBuyerContactForZuoraAccount(
      zuoraAccount: AccountSummary,
  )(implicit simpleContactRepository: SimpleContactRepository, ctx: ExecutionContext): Future[Contact] = {
    for {
  ...
  def getRecipientContactForZuoraAccount(
      zuoraAccount: AccountSummary,
  )(implicit simpleContactRepository: SimpleContactRepository, ctx: ExecutionContext): Future[Contact] = {
    for {
      sfContactId <- Some(zuoraAccount.sfContactId.get)
        .filter(_.nonEmpty)
        .fold[Future[String]](Future.failed(new IllegalStateException(s"Zuora record for ${zuoraAccount.id} has no sfContactId")))(Future.successful)
      recipientContact <- getContactById(sfContactId)
    } yield recipientContact
  }

  def getBuyerContactForSubscription(
      subscription: Subscription[AnyPlan],
  )(implicit zuoraRestService: ZuoraRestService[Future], simpleContactRepository: SimpleContactRepository, ctx: ExecutionContext): Future[Contact] = {
    for {
