...
object SubPlanReads {
  ...
  implicit val supporterPlusReads: SubPlanReads[ZSupporterPlus] = findProduct(_.supporterPlus.point[List], SupporterPlus)
  ...
  implicit def planReads[P <: Product : SubPlanReads, B <: Benefit : ChargeReads]: SubPlanReads[SubscriptionPlan[P, ChargeList with SingleBenefit[B]]] =
    new SubPlanReads[SubscriptionPlan[P, ChargeList with SingleBenefit[B]]] {
      override def read(p: ProductIds, z: SubscriptionZuoraPlan, c: CatalogZuoraPlan): ValidationNel[String, SubscriptionPlan[P, ChargeList with SingleBenefit[B]]] = {
      (
  ...
  implicit def anyPlanReads[P <: Product](implicit productReads: SubPlanReads[P], chargeListReads: ChargeListReads[ChargeList]): SubPlanReads[SubscriptionPlan[P,ChargeList]] =
    new SubPlanReads[SubscriptionPlan[P,ChargeList]] {
      override def read(ids: ProductIds, subZuoraPlan: SubscriptionZuoraPlan, catZuoraPlan: CatalogZuoraPlan): ValidationNel[String, SubscriptionPlan[P,ChargeList]] =
      {
