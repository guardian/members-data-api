...
object Readers {
  ...
  private def checkSuccess(j: JsValue): JsResult[Boolean] =
  ...
  implicit val subscriptionStatus: Reads[SubscriptionStatus] = new Reads[SubscriptionStatus] {
  ...
  implicit val featureReads: Reads[Feature] = Json.reads[Feature]
}
