...
class Discounter(discountPlans: DiscountRatePlanIds) {
  ...
  private def getRatePlanScaledToPeriod(discount: PercentDiscount, period: PeriodType, billingPeriod: BillingPeriod) = discount.durationMonths.fold {
    RatePlan(discountPlan.planId.get, ChargeOverride(productRatePlanChargeId = discountPlan.planChargeId.get, discountPercentage = discount.amount.some,endDateCondition =  SubscriptionEnd.some, billingPeriod = period.some).some)

  } { durationInMonths =>
    val (newDiscountPercent, numberOfNewPeriods) = getDiscountScaledToPeriod(discount, billingPeriod)
    val opts = ChargeOverride(productRatePlanChargeId = discountPlan.planChargeId.get,
